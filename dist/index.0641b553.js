...
async function fetchCountries(countryQuery) {
    try {
        const response = await (0, _axiosDefault.default).get("https://restcountries.com/v3.1/all?fields=name,flags,population,region,subregion,capital,currencies,languages");
        response.data.sort((a, b)=>{
            if (a.population > b.population) return 1;
        });
        listCountries(response.data, countryListElement);
        displayInfo(response.data, countryQuery, countryDisplayElement);
    } catch (e) {
        console.error(e);
    }
}
//void fetchCountries();
////////////////////////////////---PSEUDOCODE PART 2---////////////////////////////////
//Add some endpoints so I have access to the Capital and Currenies as well.
//Write HTML element for search bar and add event listener for when you press search.
//Add searchfield text as an argument to the eventlistener, and search in database for it.
...
buttonElement.addEventListener("click", ()=>{
    fetchCountries(textElement.textContent);
});
function currencySemantics(country) {
    let returnString = `${country.currencies[0].name}s`;
    for(let i = 0; i < country.currencies.length; i++){
        if (i !== 0 && i < country.currencies.length - 1) returnString += `, ${country.currencies[i].name}s`;
        else if (i !== 0 && i === country.currencies.length - 1) returnString += ` and ${country.currencies[i].name}s`;
    }
    return returnString;
}
function languageSemantics(country) {
    let returnString = `${country.languages[0]}`;
    for(let i = 0; i < country.languages.length; i++){
        if (i !== 0 && i < country.languages.length - 1) returnString += `, ${country.languages[i]}`;
        else if (i !== 0 && i === country.languages.length - 1) returnString += ` and ${country.languages[i]}`;
    }
    return returnString;
}
function displayInfo(countryLibrary, countryQuery, uiElement) {
    try {
        const country = countryLibrary.name.common.find(countryQuery);
        uiElement.innerHTML += `
...
var Buffer = require("25a09f4d99068783").Buffer;
"use strict";
/**
...
},{"25a09f4d99068783":"fCgem","../utils.js":"5By4s","../core/AxiosError.js":"3u8Tl","../platform/node/classes/FormData.js":"aFlee","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"fCgem":[function(require,module,exports) {
/*!
...
var base64 = require("a43f5984647b2228");
var ieee754 = require("46582bf0ae4882d0");
...
},{"a43f5984647b2228":"eIiSV","46582bf0ae4882d0":"cO95r"}],"eIiSV":[function(require,module,exports) {
"use strict";
